...
central_difference_array = np.zeros(
    len(h_array)
)  # Initialize array to store central difference results
forward_difference_array = np.zeros(
    len(h_array)
)  # Initialize array to store forward difference results

x = 0.5  # point at which to differentiate

"""Analytical value"""
analyticalValue = (4 * 0.5**2 - 2) * np.exp(
    -(0.5**2)
)  # Analytical derivative of f(x) = e^(-x^2) at x = 0.5
...
for i in range(len(h_array)):
    ...
    print(
        f"h = {h}: Central Difference = {central_difference_array[i]} \t Forward Difference = {forward_difference_array[i]}"
    )


"""Finding the Relative error compared to the analytical value"""

central_relative_error = np.zeros(
    len(h_array)
)  # Initialize array for relative error of central difference
forward_relative_error = np.zeros(
    len(h_array)
)  # Initialize array for relative error of forward difference
...
for i in range(len(h_array)):
    ...
    print(
        f"h = {h}: Central Difference = {central_relative_error[i]} \t Forward Difference = {forward_relative_error[i]}"
    )
...
def higher_derivative(func, x, m, h):
    ...
    if m > 1:
        return (
            higher_derivative(func, x + h / 2, m - 1, h)
            - higher_derivative(func, x - h / 2, m - 1, h)
        ) / (h)
    else:
        return central_difference(
            func, x, h
        )  # Base case: use central difference for m=1
...
for i in range(1, 6):
    print(
        f"The derivative of order {i} is {higher_derivative(g, x, i, h)}, expected value is {2.**(i)}"
