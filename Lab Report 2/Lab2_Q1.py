...
for i in n_array:
    plt.plot(
        x_array, wavefunction(x_array, i), label=f"n = {i}"
    )  # Plot wavefunction for each n
...
def g_func(z, n):
    ...
    return (
        f_func(np.tan(z), n) / np.cos(z) ** 2
    )  # Adjust the function using change of variables from x to z
...
for n in n_array:
    ...
    integral_result = gaussianQuadrature(g_func, N, a, b, x_initial, w_initial, n)[
        2
    ]  # Integral calculated for each n
